...
https://raw.githubusercontent.com/anyehttp/quantumult-x/main/task/asai-surge.sgmodule

本地-Script定时任务
[Script]
# （默认凌晨12点 执行，如需更改请自行修改cron表达式）
阿水ai签到 = type=cron, cronexp="0 0 * * *", script-path=https://raw.githubusercontent.com/anyehttp/quantumult-x/main/task/asai.js
-----------------------------------------------
*************************
【 签到脚本使用教程 】:
*************************
...
多账号：
1.抓包ai.ashuiai.com域名下的authorization
2.打开boxjs->我的->数据查看器->在数据键输入asai_data,点击VIEW->在数据内容输入抓取到的authorization，点击保存。
...
async point() {
        try {
            ...
            if (result?.code === 200) {
                    $.log(`✅查询成功！`);
                    $.pointMsg = `✅对话总量剩余:${result?.result?.[4]?.totalValue}个 /n 绘画总量剩余:${result?.result?.[7]?.totalValue}个`;
            } else {
                $.log(`❌查询失败！`);
